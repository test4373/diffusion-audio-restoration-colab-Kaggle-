...
class STFTBridgeModel(LightningModule):
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        if not SSR_EVAL_AVAILABLE:
            print("⚠️ ssr_eval mevcut değil, test metrikleri atlanıyor")
            return
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        try:
            ...
            # SSR Eval metrikleri (opsiyonel)
            if SSR_EVAL_AVAILABLE:
                n_steps = 25
                n_samples = x_0_clean.shape[0]
                t_steps = torch.linspace(1, 1.0/n_steps, int(n_steps)).unsqueeze(0).to(x_0_corrupted.device)
                ...
                print("⚠️ ssr_eval mevcut değil, validation metrikleri atlanıyor")
